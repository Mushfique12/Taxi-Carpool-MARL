...
class CityReal:
    ...
    def __init__(self, mapped_matrix_int, order_num_dist, idle_driver_dist_time, idle_driver_location_mat, order_time_dist, order_price_dist,
        ...
        onoff_driver_location_mat[t] = [[-0.625       2.92350389]  <-- Corresponds to the grid in target_node_ids
                                        ...
                                        [-1.2         2.05588586]...]
        :real_orders: N * M (num of total grids) x 144 (total time steps) x 5 (as explained below)
    ...
    def step_assign_order_broadcast_neighbor_reward_update(self):
        ...
        :return:
        """
        # Order dispatching (2 step process)
    ...
    def step(self, dispatch_actions, generate_order=1):
        info = []
        '''**************************** T = 1 ****************************'''
        # Loop over all dispatch (re-allocation) action, change the driver distribution
        # Executes Driver re-allocation
        # When the drivers go to invalid grid, set them offline.
        save_remove_id = self.step_dispatch_invalid(dispatch_actions)

        # Order dispatching and calculate rewards (2 step process)
        ...
        # drivers finish order become available again (from pre-dispatch procedure).
        self.step_driver_status_control()

        # drivers dispatched (re-allocated) at t, arrived at t + 1, become available at t+1
        self.step_add_dispatched_drivers(save_remove_id)

        # generate order at t + 1
        # Orders(objects) are appended to the order_list in each Node Object
        if generate_order == 1:
            self.step_generate_order_real()
        else:
            moment = self.city_time % self.n_intervals
